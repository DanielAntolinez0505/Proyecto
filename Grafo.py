...
class Graph:
    ...
    def draw_vertices(self, screen):
        for vertex in self.vertices:
            pygame.draw.circle(screen, (17, 219, 106), (vertex.position[0], vertex.position[1]), 1, 0)
    def draw_aristas(self, screen):
        for vertex in self.vertices:
            aristas = self.get_adjacencies(vertex.value)
            for arista in aristas:
                destiny = arista[0]
                pygame.draw.line(screen, (94, 228, 212), (vertex.position[0], vertex.position[1]), 
                                                                (destiny.position[0], destiny.position[1]), 1)
                ...
                text_value = pygame.font.SysFont(None, 15, True).render(str(arista[1]), True, (7, 9, 42))
