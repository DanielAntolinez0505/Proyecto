...
class poker:
    ...
    def definir(self):
        CARD_WIDTH = 130
        CARD_HEIGHT = 180
        carta2 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/2.png', 2)
        carta3 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/3.png', 3)
        carta4 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/4.png', 4)
        carta5 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/5.png', 5)
        carta6 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/6.png', 6)
        carta7 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/7.png', 7)
        carta8 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/8.png', 8)
        carta9 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/9.png', 9)
        carta10 = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/10.png', 10)
        cartaA = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/A.png', 14)
        cartaJ = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/J.png', 11)
        cartak = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/K.png', 12)
        cartaQ = Imagen(0, 0, CARD_WIDTH, CARD_HEIGHT, 'imagenes/Q.png', 12)
        stack_1 = Stack([cartaA, cartak, cartaQ, cartaJ, carta4]) 
        stack_2 = Stack([carta7, carta8, carta6, carta10])
        stack_3 = Stack([carta5, carta2, carta3])
        stack_4 = Stack([carta9])
        self.stacks = [stack_1, stack_2, stack_3, stack_4]
        self.lista_ganadora = [cartaA, cartak, cartaQ, cartaJ, carta10, carta9, carta8, carta7, carta6, carta5, carta4, carta3, carta2]
        hitbox1 = hitbox(pg.Rect(80, 95, 135, 550))
        hitbox2 = hitbox(pg.Rect(240, 95, 135, 550))
        hitbox3  = hitbox(pg.Rect(400, 95, 135, 550))
        hitbox4 = hitbox(pg.Rect(560, 95, 135, 550))
    ...
    def draw(self):
            ...
            for aux in stack.pila:
                if counter == 0: 
                    ...
                    if aux != None:
                        ...
                        delta4+=30
            counter+=1
        card_back = Imagen(714, 180, CARD_WIDTH, CARD_HEIGHT, 'img/back.png', 0)
        ...
        if self.stacks[0].ganador(self.lista_ganadora) or self.stacks[1].ganador(self.lista_ganadora) or self.stacks[2].ganador(self.lista_ganadora) or self.stacks[3].ganador(self.lista_ganadora):
            self.ganador()  
            self.active = False  
        
    def fondo_poker(self):
        '''Fondo'''
        fondo = Imagen(0, 0, self.width, self.height, 'img/fondo.jpg', 0)
    ...
    def eventos(self, evento : pg.event):
        if evento.type == MOUSEBUTTONDOWN:
            if self.hitbox[0].rect.collidepoint(mouse.get_pos()):
                if self.hitbox[0].active == False and self.hitbox[1].active == False and self.hitbox[2].active == False and self.hitbox[3].active == False:
            ...
            if self.hitbox[1].rect.collidepoint(mouse.get_pos()):
                if self.hitbox[1].active == False and self.hitbox[0].active == False and self.hitbox[2].active == False and self.hitbox[3].active == False:    
            ...
            if self.hitbox[2].rect.collidepoint(mouse.get_pos()):
                if self.hitbox[2].active == False and self.hitbox[1].active == False and self.hitbox[0].active == False and self.hitbox[3].active == False:
    ...
    def ganador(self):
        '''Fondo'''
        fondo = Imagen(0, 0, self.width, self.height, 'img/win.jpg', 0)
        temp = image.load(fondo.route)
        temp = transform.scale(temp, (fondo.width, fondo.height))
        self.SCREEN.blit(temp, (fondo.x, fondo.y))
...
class arbol:
    ...
    def definir(self):
        ...
        self.text_field = CuadroTxt('valor', 20, 40, 120, 23)
    ...
    def fondo_arbol(self):
        '''Fondo'''
        fondo = Imagen(0, 0, self.width, self.height, 'img/bosque.jpg', 0)
        temp = image.load(fondo.route)
        temp = transform.scale(temp, (fondo.width, fondo.height))
        self.SCREEN.blit(temp, (fondo.x, fondo.y))
    ...
    def dibujar_arbol(self):
        ...
        def dfs(node:Nodo, level, xCoordenada, xDelta, r):
    ...
    def show_traversal(self):
        ...
        if self.combo_box.selected_item == 'InOrden':
            ...
            pg.draw.rect(self.SCREEN, (145, 171, 103), (self.SCREEN.get_width()//2 - txt.get_width()//2 - 15, self.SCREEN.get_height()-100, 
...
class grafo:
    def __init__(self, SCREEN, width, height):
        ...
        self.peso_input = CuadroTxt("peso", 70, self.height - 40, 100, 30)
        self.mi_grafo = None
    ...
    def definir(self):
        ...
        self.btn_dijkstra = boton(self.width - 160, 350, 150, 50, "DIJKSTRA")
        
        
    def fondo_pantalla(self):
        '''Fondo'''
        self.SCREEN.fill((255,255,255))
        fondo = Imagen(250, 0, 500, 650, 'img/mapa.jpeg', 0)
        temp = image.load(fondo.route)
        temp = transform.scale(temp, (fondo.width, fondo.height))
        self.SCREEN.blit(temp, (fondo.x, fondo.y))
